...
func (likeDao *LikeDao) GetLikeByUserIDAndVideoID(UserID int64, VideoId int64) (Favorite, error) {
	f := Favorite{}
	result := Db.Table("favorite").Where("user_id = ? and video_id = ?", UserID, VideoId).Take(&f)
	//错误处理
	if result.Error != nil {
...
func (likeDao *LikeDao) InsertLike(UserID int64, VideoId int64) error {
	...
	if err := tx.Table("favorite").Select("user_id", "video_id").Create(&favorite).Error; err != nil {
		log.Println("添加点赞回滚！错误信息：", err)
		tx.Rollback()
	}
	video := &Video{ID: VideoId}
	if err := tx.Table("video").Model(&video).UpdateColumn("favorite_count", gorm.Expr("favorite_count + 1")).Error; err != nil {
...
func (likeDao *LikeDao) DeleteLike(UserID int64, VideoId int64) error {
	...
	if err := tx.Table("favorite").Where("user_id = ? and video_id = ?", UserID, VideoId).Delete(&favorite).Error; err != nil {
		log.Println("取消点赞回滚！错误信息：", err)
		tx.Rollback()
	}

	video := &Video{ID: VideoId}
	if err := tx.Table("video").Model(&video).Where("favorite_count != 0").UpdateColumn("favorite_count", gorm.Expr("favorite_count - 1")).Error; err != nil {
		log.Println("更新视频点赞数回滚！错误信息：", err)
		tx.Rollback()
	}
	tx.Commit()
